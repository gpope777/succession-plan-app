...
const COL_GROUPS = [
  { key: 'estado',    label: 'Estado',     bg: '#e6f1fb', color: '#185FA5' },
  { key: 'desempeno', label: 'Desempeño',  bg: '#e1f5ee', color: '#0f6e56' },
  { key: 'potencial', label: 'Potencial',  bg: '#eeecfb', color: '#534AB7' },
  { key: 'nineBox',   label: '9-Box',      bg: '#faeeda', color: '#BA7517' },
  { key: 'calor',     label: 'Mapa Calor', bg: '#faece7', color: '#993C1D' },
  { key: 'idp',       label: 'IDP',        bg: '#f7f8f9', color: '#1a2e3b' },
]
...
export default function BaseDatos({ data, adminMode, updateCollaborator, updateHeatmap }) {
  ...
  const displayed = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortKey === 'perfAvg')   return (b.rubricScores?.weightedAvg ?? -1) - (a.rubricScores?.weightedAvg ?? -1)
      if (sortKey === 'potAvg')    return (b.potentialScores?.weightedAvg ?? -1) - (a.potentialScores?.weightedAvg ?? -1)
      return 0
    })
    return list
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        parsed.evaluaciones.forEach(entry => {
          ...
          if (entry.rubricScores) {
            ...
            if (entry.rubricScores.weightedAvg) {
              ...
              updates.nineBox = { ...existing.nineBox, performance: Math.max(1, Math.min(5, Math.round(entry.rubricScores.weightedAvg))) }
            }
          }
        ...
        if (parsed.heatmap && Array.isArray(parsed.heatmap)) updateHeatmap(parsed.heatmap)
    ...
    <div style={{ maxWidth: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--navy)', marginBottom: 3 }}>
            🗄️ Base de Datos de Evaluaciones
          </h1>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <button className="btn btn-white btn-sm" onClick={() => exportEvaluacionesCSV(data)}>📊 Exportar CSV</button>
          <button className="btn btn-white btn-sm" onClick={() => exportEvaluacionesJSON(data)}>💾 Exportar JSON</button>
          {adminMode && (
            <>
              <button
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <select
            value={filterCollab} onChange={e => setFilterCollab(e.target.value)}
        ...
        <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 12 }}>
          <thead>
            <tr>
              ...
              {visible.estado    && <th colSpan={4} style={{ ...TH_BASE, background: '#e6f1fb', color: '#185FA5' }}>Estado Readiness</th>}
              {visible.desempeno && <th colSpan={7} style={{ ...TH_BASE, background: '#e1f5ee', color: '#0f6e56' }}>Rúbricas — Desempeño</th>}
              {visible.potencial && <th colSpan={5} style={{ ...TH_BASE, background: '#eeecfb', color: '#534AB7' }}>Rúbricas — Potencial</th>}
              {visible.nineBox   && <th colSpan={2} style={{ ...TH_BASE, background: '#faeeda', color: '#BA7517' }}>9-Box</th>}
              {visible.calor     && <th colSpan={heatmap.length} style={{ ...TH_BASE, background: '#faece7', color: '#993C1D' }}>Mapa de Calor</th>}
              {visible.idp       && <th colSpan={3} style={{ ...TH_BASE, background: '#f7f8f9', color: '#1a2e3b' }}>IDP</th>}
            </tr>
            <tr>
              {visible.estado && <>
              ...
              {visible.calor && heatmap.map(row => (
                <th key={row.dimension} style={{ ...TH_BASE, background: '#FDF1ED', color: '#993C1D', top: 28, maxWidth: 90 }} title={row.dimension}>
              ...
              return (
                <tr key={c.id} style={{ background: rowBg }}>
                  ...
                  {visible.estado && <>
                    ...
                    <td style={TD_BASE}>
                      <span style={{ fontWeight: 700, fontSize: 13, color: c.readinessPercentage >= 70 ? '#0f6e56' : c.readinessPercentage >= 45 ? '#BA7517' : '#A32D2D' }}>
                  ...
                  {visible.desempeno && <>
                    {PERF_DIMS.map(d => (
                      <td key={d.id} style={TD_BASE}><ScoreChip value={rs?.scores?.[d.id] ?? null} /></td>
                    ))}
                  ...
                  {visible.potencial && <>
                    {POT_DIMS.map(d => (
                      <td key={d.id} style={TD_BASE}><ScoreChip value={ps?.scores?.[d.id] ?? null} /></td>
                    ))}
                  ...
                  {visible.calor && heatmap.map(row => (
                    <td key={row.dimension} style={TD_BASE}><ScoreChip value={row[c.id] ?? null} /></td>
                  ))}
